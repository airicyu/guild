...
import { missionRoomPath, poSessionName, intakeLeadSessionName } from "../../paths";
...
export async function spawnDiscoveryLead(
  ...
  const sessionName = intakeLeadSessionName(ideaId);
  const cwd = missionRoomPath(config, ideaId);
  return spawnBackgroundSession(config, { sessionName, cwd, prompt });
}
