...
import { assertNoteId } from "./note-id";
import { isSessionRunning } from "./session";

export { assertNoteId as assertMissionId } from "./note-id";
...
/** Discovering entries with a live intake lead session (consumes a slot). */
export async function countDiscoveringSessions(config: Config): Promise<number> {
  ...
  for (const noteId of discovering) {
    const checkpoint = await readCheckpoint(config, noteId);
    if (!checkpoint || checkpoint.phase === "mission_plan_complete") continue;
...
/** Find board stage for a note id; null if not on board. */
export function resolveBoardNoteStage(board: BoardListing, noteId: string): BoardStage | null {
  for (const stage of BOARD_STAGES) {
    if (board[stage].includes(noteId)) return stage;
  }
  return null;
}
