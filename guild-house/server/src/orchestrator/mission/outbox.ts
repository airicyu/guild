...
async function missionOutboxPath(config: Config, missionId: string): Promise<string> {
  return outboxPath(config, missionId);
}
...
export async function readMissionOutbox(config: Config, missionId: string): Promise<OutboxEntry[]> {
  ...
  return readJsonl<OutboxEntry>(await missionOutboxPath(config, missionId));
}
...
export async function appendOutboxEntry(
  ...
  await appendJsonl(await missionOutboxPath(config, missionId), entry);
  return entry;
}
...
async function removeOutboxEntry(config: Config, missionId: string, entryId: string): Promise<void> {
  ...
  await writeJsonl(
    await missionOutboxPath(config, missionId),
...
export async function escalateToGuildMaster(
  ...
  const signalType =
    checkpoint && (checkpoint.mode === "intake" || isIntakePhase(checkpoint.phase))
      ? ("awaiting_input" as const)
      : ("blocked" as const);

  try {
    const updated = await handleSignal(config, missionId, {
      type: signalType,
      by: from,
      summary: input.question,
    });
    return { entry, checkpoint: updated };
  } catch (err) {
...
export async function listUnreadOutbox(config: Config): Promise<OutboxItem[]> {
  ...
  for (const missionId of [...board.working, ...board.archive, ...board.discovering]) {
    const entries = await readMissionOutbox(config, missionId);
    for (const entry of entries) {
      if (!entry.read) {
        items.push(
          board.discovering.includes(missionId)
            ? { ...entry, ideaId: missionId }
            : { ...entry, missionId },
        );
      }
    }
...
export async function markOutboxRead(
  ...
  await writeJsonl(await missionOutboxPath(config, missionId), entries);
