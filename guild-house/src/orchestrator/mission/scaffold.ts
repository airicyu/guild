...
async function copyMissionBrief(config: Config, missionId: string, roomPath: string): Promise<void> {
  ...
  const commonDir = join(roomPath, "memories", "common");
  await mkdir(commonDir, { recursive: true });
  await cp(briefPath, join(commonDir, "mission-brief.md"));
}
...
export async function ensureMissionBriefInRoom(config: Config, missionId: string): Promise<void> {
  const roomPath = missionRoomPath(config, missionId);
  const target = join(roomPath, "memories", "common", "mission-brief.md");

  try {
    await stat(target);
    return;
  } catch {
    // missing — try board copy
  }
...
async function writeHandoffPrompt(config: Config, missionId: string, roomPath: string): Promise<void> {
  ...
  try {
    ...
    await writeFile(join(guildDir, "mission-schema.md"), applyTemplateVars(schema, vars), "utf8");
  } catch {
...
export function initialSpawnPrompt(missionId: string): string {
  return [
    ...
    "Start by reading memories/common/mission-brief.md and members/project-owner/agent.md.",
