...
/** Handle OPTIONS preflight; null when not a CORS preflight request. */
export function corsPreflight(config: Config, req: Request): Response | null {
  if (req.method !== "OPTIONS") return null;
  return new Response(null, {
    status: 204,
    headers: corsHeaders(config, req),
  });
}
