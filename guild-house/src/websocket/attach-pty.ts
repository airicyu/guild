...
import { ensureDiscoverySessionLive } from "../orchestrator/discovery/session-lifecycle";
...
export function extractAttachRoute(pathname: string): AttachRoute | null {
  ...
  if (discoveryMatch) {
    return { pipeline: "discovery", resourceId: decodeURIComponent(discoveryMatch[1]) };
  }
  return null;
}

/** @deprecated Use extractAttachRoute — mission id only. */
export function extractAttachMissionId(pathname: string): string | null {
  const route = extractAttachRoute(pathname);
  return route?.pipeline === "mission" ? route.resourceId : null;
}
...
async function ensureSessionLive(
  ...
  if (pipeline === "discovery") {
    const result = await ensureDiscoverySessionLive(config, resourceId);
...
function notLiveMessage(pipeline: AttachPipeline, restoreRequired: boolean): string {
  if (pipeline === "discovery") {
    return restoreRequired
      ? "Discovery session is not live — restore the session first"
      : "Discovery session is not available for attach";
  }
  return restoreRequired
    ? "PO session is not live — restore the session first"
    : "PO session is not available for attach";
}
